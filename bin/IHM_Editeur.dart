...
class DBEditeur {
  static Future<Editeur> selectEditeur(int id) async {
    ...
    try {
      ...
      try {
        String requete = "SELECT * FROM Editeur WHERE id=" + id.toString() + " AND EXISTS (SELECT id FROM Editeur WHERE id=" + id.toString() + " );";
        Results reponse = await conn.query(requete);
        edi = Editeur(reponse.first['id'], reponse.first['nom'], reponse.first['adresse']);
      } catch (e) {
        log(e.toString());
      }
      conn.close();
  ...
  static Future<void> insertEditeur(String nom, String adresse) async {
    try {
      MySqlConnection conn = await MySqlConnection.connect(DBConfig.getSettings());
      try {
        String requete = "INSERT INTO Editeur (nom, adresse) VALUES('" + nom + "', '" + adresse + "');";
  ...
  static Future<void> updateEditeur(int id, String nom, String adresse) async {
    try {
      MySqlConnection conn = await MySqlConnection.connect(DBConfig.getSettings());
      try {
        String requete = "UPDATE Editeur SET nom = '" + nom + ", adresse = '" + adresse + ", ' WHERE id='" + id.toString() + "'";
        await conn.query(requete);
      } catch (e) {
        log(e.toString());
      }
  ...
  static Future<Editeur> getEditeur(int id) async {
    ...
    return Auteur(rr['id'], rr['nom'], rr['adresse']);
  }
}