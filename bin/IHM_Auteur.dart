...
class DBAuteur {
  static Future<Auteur> selectAuteur(int id) async {
    ...
    try {
      ...
      try {
        String requete = "SELECT * FROM Auteur WHERE id=" + id.toString() + " AND EXISTS (SELECT id FROM Auteur WHERE id=" + id.toString() + " );";
        Results reponse = await conn.query(requete);
        aut = Auteur(reponse.first['id'], reponse.first['nom'], reponse.first['prenom']);
      } catch (e) {
        log(e.toString());
      }
      conn.close();
  ...
  static Future<void> insertAuteur(String nom, String prenom) async {
    try {
      MySqlConnection conn = await MySqlConnection.connect(DBConfig.getSettings());
      try {
        String requete = "INSERT INTO Auteur (nom, prenom) VALUES('" + nom + "', '" + prenom + "');";
  ...
  static Future<void> updateAuteur(int id, String nom, String prenom) async {
    try {
      MySqlConnection conn = await MySqlConnection.connect(DBConfig.getSettings());
      try {
        String requete = "UPDATE Auteur SET nom = '" + nom + ", prenom = '" + prenom + ", ' WHERE id='" + id.toString() + "'";
        await conn.query(requete);
      } catch (e) {
        log(e.toString());
      }
